...
*Homopolymers* consist of a stretch of repeated nucleotides (i.e. $\geq 2$) occurring at some point in the genome. For example the sequence AAAA is a length 4 adenine homopolymer.

```{r, HPpercent, fig.cap="Homopolymer length as fraction of the genome", label="HPpercent",out.width="60%"}
...
```{r, HPdistrib, fig.cap=hpDistribCaption, label="HPdistrib", out.width="60%", cache=FALSE}
...
-   Used in many software tools:
    ...
    -   assembly: HiCanu [@nurkHiCanuAccurateAssembly2020], MDBG [@ekimMinimizerspaceBruijnGraphs2021], pyrosequencing assembly [@millerAggressiveAssemblyPyrosequencing2008], wtdbg2 [@ruanFastAccurateLongread2020], shasta [@shafinNanoporeSequencingShasta2020]

    -   clustering: [@sahlinNovoClusteringLongRead2020]

    -   error correction: LSC [@auImprovingPacBioLong2012] and LSCPlus [@huLSCplusFastSolution2016]

    -   alignment: minimap2 [@liMinimap2PairwiseAlignment2018], winnowmap2 [@jainWeightedMinimizerSampling2020].

    -   Satellite tandem repeat analysis [@vannesteForensicSTRAnalysis2012]
...
-   **Sequencing is an active field:** new methods, both in short reads (ultima [@almogyCostefficientWholeGenomesequencing2022] ) and long reads (illumina infinity [@HighPerformanceLong] ), as well as improving existing technologies, PacBio (by acquiring Omniome [@ShortreadSequencingBinding]) developing a new accurate short read technology [@cetinPlasmonicSensorCould2018]
