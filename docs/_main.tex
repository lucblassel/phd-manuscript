...
\item
  ...
  \href{https://google.com}{DOI goes here} \emph{(Included in Chapter} \ref{HPC-paper}\emph{)}
\item
  ...
  \href{https://doi.org/10.1016/j.coviro.2021.09.009}{10.1016/j.coviro.2021.09.009}
\item
  ...
  \href{https://doi.org/10.1371/journal.pcbi.1008873}{10.1371/journal.pcbi.1008873}. \emph{(Included in Chapter} \ref{HIV-paper}\emph{)}
\item
...
This sequence is directly dependent on the information contained in the DNA. First the DNA is transcripted in a similar, but single stranded, molecule called RNA which encodes the same sequence. This RNA molecule is then translated into a protein by the following process \autocite{crick1961}:

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  Nucleotides in the RNA sequence are read in groups of 3 called a codon.
\item
...
\item
  The translation process is stopped when a specific type of codon is read.
\end{enumerate}
...
\begin{itemize}
\item
  \textbf{DNA replication:} When a cell divides, or when an organism reproduces, the DNA molecule must be copied in order to transmit genetic information. This process has a very low rate of errors, with as low as 1 error for every billion to every hundred billions of base pairs replicated \autocite{fijalkowska2012}. This is due to the fact that the DNA polymerase (the protein that is responsible for copying DNA molecules), has a relatively error rate to start with, but mostly due to the error correcting mechanisms that are present in certain cells and bacteria \autocite{pray2008dna}.
\item
  \textbf{RNA transcription:} error rate between 4 errors for each million \autocite{gout2013} to 2 errors for each hundred thousand \autocite{gout2017} base pairs transcribed.
\item
  \textbf{Other mutagenic events:} Ionizing radiation \autocite{desouky2015}, UV rays \autocite{kiefer2007}, Toxins \autocite{bennett2003}, heat Stress \autocite{kantidze2016}, cold stress \autocite{gregory1994}, oxidative stress \autocite{gafter-gvili2013}.
\end{itemize}
...
\item
  ...
  \begin{itemize}
  \item
    Pathologic coagulation linked to a single mutation in DNA coding for a coagulation protein \autocite{kujovichFactorLeidenThrombophilia2011}.
  \item
    Cystic Fibrosis, a deletion of a single amino acid in the protein leads to fatal disease \autocite{cuttingCysticFibrosisGenetics2015}
  \item
    Plenty of mutation in specific genes have been linked with type 2 diabetes \autocite{fuchsbergerGeneticArchitectureType2016,morrisLargescaleAssociationAnalysis2012}
  \end{itemize}
\item
  ...
  \begin{itemize}
  \item
    Dozens of mutations proven to confer HIV resistance to commonly used drugs \autocite{rheeHumanImmunodeficiencyVirus2003}
  \item
    A large amount of mutations have been shown to confer antibacterial resistance in a range of bacteria \autocite{woodfordEmergenceAntibioticResistance2007}
  \end{itemize}
\end{itemize}
...
The first true sequencing method was developed in 1977 \autocite{sangerDNASequencingChainterminating1977}. Sanger \emph{et al.} devised a simple method to read the sequence of nucleotides that make up a DNA sequence \emph{(also represented in Figure} \ref{fig:sanger}\emph{)}.

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  Clone sequence / amplify
\item
  Prepare 4 different sequencing environments with a majority of dNTP (ie regular nucleotides) and in each a single type of ddNTP (a terminator). ddNTP are marked
\item
  In each test tube add DNA polymerase, primers and denatures DNA fragments you want to sequence
\item
  Sequence is replicated until incorporation of ddNTP stopping reaction
\item
  Separate replicated fragments by gel electrophoresis (i.e shorter fragments go further), 1 ddNTP type in each lane
\item
  With marked you can see which nucleotide is present at a given position
\end{enumerate}

This allowed Sanger \emph{et al.} to sequence the first whole \(\varphi X174\) bacteriophage genome \autocite{sangerNucleotideSequenceBacteriophage1977}. This method, although revolutionnary was costly and time consuming.

The marking of primers and ddNTP with fluorescence allowed to do the polymerization in a single test tube and use a single lane for electrophoresis \autocite{smithSynthesisOligonucleotidesContaining1985,smithFluorescenceDetectionAutomated1986}. The fluorescence also allowed for automated reading with optical systems. Plus switching to capillary electrophoresis allowed for automation and speeding up. Shotgun sequencing also speed up stuff at the reconstruct the whole genome.
...
With the latest Sanger sequencing technologies, sequenced sequences can reach 1kBp with an accuracy of 99.999\% \autocite{shendureNextgenerationDNASequencing2008}. Although very time consuming and expensive at it's dawn, the sequencing of the first human genome costing between 500 million and 1 billion dollars \autocite{CostSequencingHuman}. Over time and with technological advancments, the cost of Sanger sequencing was greatly reduced from \$1000 per Bp \autocite{collinsHumanGenomeProject2003} to \$0.5 per kBp \autocite{shendureNextgenerationDNASequencing2008}, and the throughput increased from 1 kBp per day \autocite{collinsHumanGenomeProject2003} to up to 120 kBp per hour \autocite{liuComparisonNextGenerationSequencing2012}.
...
\begin{itemize}
\item
  Massively parallel \emph{(no need to detail technology, just numbers i.e.~throughput and cost) (i.e.~2nd gen)} \autocite{metzkerSequencingTechnologiesNext2010}
  ...
  \item
    ...
    \begin{itemize}
    \item
      150nt \autocite{mardisDecadePerspectiveDNA2011}
    \item
      98\% accuracy, \$0.07 per MBp, 2.5 to 12.5 Gbp per hour \autocite{liuComparisonNextGenerationSequencing2012}
    \item
  ...
  \item
    ...
    \begin{itemize}
    \item
      400nt \autocite{mardisDecadePerspectiveDNA2011}
    \item
      99.9\% accuracy, \$10 per MBp, 30Mbp per hour \autocite{liuComparisonNextGenerationSequencing2012}
    \item
...
\item
  ...
  \item
    ...
    \begin{itemize}
    \item
      Better for analyzing complex and repetitive parts of the genome.
    \item
    ...
    \end{itemize}
  \item
    PacBIO SMRT \autocite{eidRealTimeDNASequencing2009}:
    ...
    \item
      ...
      \end{itemize}
    \item
      Most reads around 10kb in length with reads up to 60kb \autocite{rhoadsPacBioSequencingIts2015}
    \item
      85\% to 92\% accuracy \autocite{chaissonResolvingComplexityHuman2015,logsdonLongreadHumanGenome2020}
    \item
      \$0.32 per Mbp \autocite{murigneuxComparisonLongreadMethods2020}
    \item
      2Gbp to 11Gbp per hour \autocite{logsdonLongreadHumanGenome2020}
    \end{itemize}
  \item
    NanoPore \autocite{clarkeContinuousBaseIdentification2009}:
    ...
    \item
      ...
      \item
        Disruptions are characteristic of the 5/6 bases in the nanopore channel -\textgreater{} deduce sequence from current disruptions (i.e.~base calling)
      \end{itemize}
    \item
      Median read length around 10kb - 12kb \autocite{ipMinIONAnalysisReference2015,logsdonLongreadHumanGenome2020}.
    \item
      you can get ultra long reads, of almost 1Mb \autocite{jainNanoporeSequencingAssembly2018,TharSheBlows} and even up to 2.3 Mb \autocite{payneBulkVisGraphicalViewer2019}.
    \item
      87\% to 98\% accuracy \autocite{jainOxfordNanoporeMinION2016,logsdonLongreadHumanGenome2020}
    \item
      \$0.13 per Mbp \autocite{murigneuxComparisonLongreadMethods2020}
    \item
      12.5Gbp to 260Gbp per hour !! \autocite{logsdonLongreadHumanGenome2020}
    \end{itemize}
  \end{itemize}
\end{itemize}
...
\begin{itemize}
\item
  has been going on for a long time \autocite{huntProteinSequencingTandem1986,smithProteinSequencingProtocols2002} with mass spectrometry
\item
  But still ongoing with new methods \autocite{restrepo-perezPavingWaySinglemolecule2018}
\end{itemize}
...
general context of sequencing errors, i.e.~short reads have high accuracy (After computational correction you can get 10\textsuperscript{-4} to 10\textsuperscript{-5} error rate \autocite{maAnalysisErrorProfiles2019} ) , long reads high error rate as stated before, 10\% to 15\%.
...
\begin{itemize}
\item
  In Non-Hybrid error correcting methods only the long reads are used, by overlapping them and taking the consensus of the overlap it is possible to eliminate some errors, as well as filtering very rare subsequences likely due to a sequencing error \autocite{limaComparativeAssessmentLongread2020,amarasingheOpportunitiesChallengesLongread2020}. These methods are applied in several pieces of software like wtdbg2 \autocite{ruanFastAccurateLongread2020}, canu \autocite{korenCanuScalableAccurate2017}, or daccord \autocite{tischlerNonHybridLong2017}.
\item
  In Hybrid methods, shorter more accurate reads are used to correct long-read errors, it has been used in software like proovread \autocite{hacklProovreadLargescaleHighaccuracy2014}, Jabba \autocite{miclotteJabbaHybridError2016}, PBcR \autocite{korenHybridErrorCorrection2012} or LoRDEC \autocite{salmelaLoRDECAccurateEfficient2014}
\item
  In some cases it can be useful to correct errors after having assembled the raw reads in a process called polishing. One can use non-hybrid methods by filtering out rare subsequences like in ntEdits , or correcting with raw PacBio or ONT data as is done in Arrow \autocite{heplerImprovedCircularConsensus2016} or Nanopolish \autocite{simpsonDetectingDNACytosine2017} respectively. Conversly, hybrid methods using short reads also exist to correct assembled reads like Pilon \autocite{walkerPilonIntegratedTool2014} or Racon \autocite{vaserFastAccurateNovo2017}.
\item
  Error correction bibliography is rich and many people are working on this \autocite{limaComparativeAssessmentLongread2020,fuComparativeEvaluationHybrid2019,zhangComprehensiveEvaluationLong2020}
\end{itemize}
...
\begin{itemize}
\item
  CCS / HiFi stuff -\textgreater{} 99.8\% to 99.9\% accuracy on long reads \autocite{wenger2019,logsdonLongreadHumanGenome2020} because errors are randomly distributed. Most errors are still indels in homopolymers \autocite{wenger2019}
\item
  Using unique molecular identifiers and consensus reads, you can reduce error rate to 99.59\% and 99.93\% accuracies for ONT and PacBio CCS \autocite{karstHighaccuracyLongreadAmplicon2021}.
\item
  Improve base calling models for ONT by switching from HMMs to deep learning, \autocite{peresiniNanoporeBaseCalling2021,bozaDeepNanoDeepRecurrent2017,wickPerformanceNeuralNetwork2019,amarasingheOpportunitiesChallengesLongread2020}
\item
  Built in error correction \autocite{chenHighlyAccurateFluorogenic2017}, results in shorter reads with 99.82\% accuracy (200-250nt reads though). Error free up to 200bp.
\item
  To come Illumina Infinity \autocite{HighPerformanceLong}, high throughput long reads.
\end{itemize}
...
\hypertarget{homopolymers-and-the-genome}{%
\subsection{Homopolymers and the genome}\label{homopolymers-and-the-genome}}

\begin{itemize}
\tightlist
\item
  What are homopolymers -\textgreater{} repeated stretches of identical nucleotides
\item
  On the CHM13 whole human genome assembly v1.1, 50\% of the 3Gb are in homopolymers of size length or more, and 10\% are in homopolymers fof size 4 or more. The longest homopolymer in the CHM13 v1.1 whole human genome assembly is 86. Homopolymers of short/medium lengths make up a significant part of the human genome (Figure \ref{fig:HPpercent}).
\item
  HPs are more often A/T rich than G/C rich in the human genome (Figure \ref{fig:HPdistrib}).
\item
  More than 1.4 million homopolymers 4-mers and up in the exome (i.e.~DNA that is translated into proteins) \autocite{ivadyAnalyticalParametersValidation2018}
\item
  according to \autocite{booeshaghiPseudoalignmentFacilitatesAssignment2022}, in the GRCh38 human genome assembly, more than 1.9 Mb are in homopolymers of length 8 or higher, representing about a thousandth of the genome, still a lot of bases to make mistakes on! Longest HP is 90.
\end{itemize}

\begin{figure}

{\centering \includegraphics[width=0.6\linewidth]{_main_files/figure-latex/HPdistrib-1} 

}

\caption{Distribution of homopolymer lengths per base in the human genome (for homopolyers of length $\geq 4$.)}\label{fig:HPdistrib}
\end{figure}

\begin{figure}

{\centering \includegraphics[width=0.6\linewidth]{_main_files/figure-latex/HPpercent-1} 

}

\caption{Homopolymer length as fraction of the genome}\label{fig:HPpercent}
\end{figure}

\hypertarget{homopolymers-and-long-reads}{%
\subsection{Homopolymers and long reads}\label{homopolymers-and-long-reads}}

Mainly indels, in certain regions of the genomes, particularly homopolymers. For ONT \autocite{delahaye2021,goodwin2015}, for PacBIO and ONT \autocite{dohmBenchmarkingLongreadCorrection2020,weirather2017}. ONT has more homopolymer errors than PacBIO \autocite{logsdonLongreadHumanGenome2020}. For short reads and PacBio the error rate usually goes up as the homopolymer length grows, however ONT's error rate in HPs does not depend on the length (even though it's the highest it's flat) \autocite{fooxPerformanceAssessmentDNA2021}.
...
\tightlist
\item
  In some cases homopolymers are taken into special consideration during polishing \autocite{huangHomopolishMethodRemoval2021,simpsonDetectingDNACytosine2017}, and computational approaches are developed to deal with homopolymers \autocite{rangSquiggleBasepairComputational2018,sarkozyCallingHomopolymerStretches2018}
\item
  Specific technologies:

  \begin{itemize}
  \item
    R.10 Nanopore: \autocite{R10NewestNanopore,amarasingheOpportunitiesChallengesLongread2020}
  \item
    Solid state nanopores: \autocite{zhouDetectionDNAHomopolymer2019,gotoIdentificationFourSinglestranded2018}
  \item
    BaseCallers which deal with homopolymers, like can be seen in Fig 3 of \autocite{wickPerformanceNeuralNetwork2019} (scrappie raw 1.4.1 or guppy)
  \end{itemize}
\item
  Avoid using HPs in barcode sequences \autocite{hawkinsIndelcorrectingDNABarcodes2018,srivathsanMinIONBasedPipeline2018}.
\item
  A little different but HPs have to be taken into account when developing DNA based data storage \autocite{wangConstructionBioConstrainedCode2019}
\end{itemize}

\hypertarget{hpc-trick}{%
\subsubsection{HPC Trick}\label{hpc-trick}}

homopolymers are still tricky regions even with long high accuracy reads -\textgreater{} so HPC
...
\item
  Used in many software tools:

  \begin{itemize}
  \item
    assembly: HiCanu \autocite{nurkHiCanuAccurateAssembly2020}, MDBG \autocite{ekimMinimizerspaceBruijnGraphs2021}, pyrosequencing assembly \autocite{millerAggressiveAssemblyPyrosequencing2008}, wtdbg2 \autocite{ruanFastAccurateLongread2020}, shasta \autocite{shafinNanoporeSequencingShasta2020}
  \item
    clustering: \autocite{sahlinNovoClusteringLongRead2020}
  \item
    error correction: LSC \autocite{auImprovingPacBioLong2012} and LSCPlus \autocite{huLSCplusFastSolution2016}
  \item
    alignment: minimap2 \autocite{liMinimap2PairwiseAlignment2018}, winnowmap2 \autocite{jainWeightedMinimizerSampling2020}.
  \item
    Satellite tandem repeat analysis \autocite{vannesteForensicSTRAnalysis2012}
  \end{itemize}
\end{itemize}
...
A read with homopolymer related sequencing errors can be homologous to two different regions of the reference genome. After applying HPC, this ambiguity is resolved and the read is homologous to only one region.
}
\label{fig:hpcSchema}
\end{figure}
...
\begin{itemize}
\item
  \textbf{Sequencing is an active field:} new methods, both in short reads (ultima \autocite{almogyCostefficientWholeGenomesequencing2022} ) and long reads (illumina infinity \autocite{HighPerformanceLong} ), as well as improving existing technologies, PacBio (by acquiring Omniome \autocite{ShortreadSequencingBinding}) developing a new accurate short read technology \autocite{cetinPlasmonicSensorCould2018}
\item
  \textbf{Long reads are good:} Super useful for plenty of very important tasks but\ldots{}
\item
  \textbf{Errors are bad:} particularly in HPs since they are an important part of the genome and a major source of error in long reads
\item
  \textbf{Computational view:} sequences Essentially a text file. Just a long succession of letters, so we can apply String/text algorithmics to it.
\end{itemize}
...
\item
  ...
  \item
    Many ways of finding seeds \autocite{sunChoosingBestHeuristic2006} and to index them \autocite{liSurveySequenceAlignment2010}
  \item
    2 use cases:
...
\item
  ...
  \end{itemize}
\item
  Both methods are very Fast, taking only a couple seconds to find approximate local alignments for 100 sequences \autocite{saripellaBenchmarkingNextGeneration2016} in a database of over 80 million sequences \autocite{finnPfamProteinFamilies2016}, with HMMs being quite slow. It is much faster to use these than to try to do the same task with SW or NW \autocite{essoussiComparisonFourPairwise2007}.
\item
  Both BLAST and FASTA compute a statistical p-value for a pairwise alignment and an expected number of random hits when scanning the database (E-value) \autocite{karlinMethodsAssessingStatistical1990a,mottAlignmentStatisticalSignificance2005}
\item
...
\emph{RC-insensitive} if for all \(x\), \(f(RC(x))= RC(f(x))\). Observe that HPC is
RC-insensitive.
...
as-is, since we do not apply \(g\) on them (as per Equation \eqref{eq:MSR}). As we
...
\item
  ...
  \begin{matrix}
      ...
      j
  \end{matrix}
  \bigg) }\) ways to choose which \(j\) \(\ell\)-mers are in \(S_0\), and
  \(\tiny{\bigg\{%
  \begin{matrix}
      i(\ell) - j \\
      k
  \end{matrix}
  \bigg\} }\) ways to partition the remaining \(\ell\)-mers into
  \(S_1, \ldots, S_k\). Hence,
  \(C(\ell, k,j) = \tiny{\bigg(
  \begin{matrix}
      i(\ell) \\
...
\begin{itemize}
\item
  \textbf{MSR}\(_{\text{F}}\): The MSR with the highest fraction of mapped reads at a
  mapq threshold of 0.
\item
  \textbf{MSR}\(_{\text{E}}\): The MSR with the lowest error rate at its respective
  mapq threshold.
\item
  \textbf{MSR}\(_{\text{P}}\): The MSR with the highest percentage of mapq thresholds
...
\begin{itemize}
\item
  Viruses are intracellular parasites \autocite{minorViruses2014}
\item
  Weird in between, are they organisms or molecules ?
\item
  André Lwoff, famous Pasteur Virologist gave the most apt definition: \emph{``viruses are viruses''} \autocite{lwoffConceptVirus1957}, there are however some key characteristics:
  ...
  \item
    can be pathogenic (HIV, SARS-CoV, Influenza, \ldots) or not (GBV-C \autocite{stapletonGBVirusesReview2011} , a strain of H5N1 influenza \autocite{yamamotoCharacterizationNonpathogenicH5N12011} )
  \end{itemize}
\item
  ...
  \item
    ...
    \begin{itemize}
    \item
      vertebrates \autocite{shiEvolutionaryHistoryVertebrate2018}
    \item
      invertrebrates \autocite{adamsAtlasInvertebrateViruses2017}
    \end{itemize}
  \item
    Plants \autocite{lefeuvreEvolutionEcologyPlant2019}
  \item
    Bacteria \autocite{delbrockBacterialVirusesBacteriophages1946}, phages being considered as therapy option separate to antibiotics \autocite{clarkBacterialVirusesHuman2004,vankan-davelaarUsingVirusesNanomedicines2014}
  \item
    Archea \autocite{prangishviliVirusesArchaea2016,prangishviliVirusesArchaeaUnifying2006}
  \item
    Virus of viruses like plant satellite viruses \autocite{franckiPlantVirusSatellites,xuPlantVirusSatellites2011} and hepatitis delta virus \autocite{laiMolecularBiologyHepatitis1995,hughesHepatitisDeltaVirus2011}. In both cases these ``viroids'' cannot replicate on theire own they must have a co-infection with the larger virus to replicate.
  \end{itemize}
\item
  Where and when viruses originated is still under study \autocite{nasirInvestigatingConceptOrigin2020,forterreOriginViruses2009}, but they may have played an important role in emergence of eukaryotic cells \autocite{forterreOriginVirusesTheir2006}.
\item
  In any case, because of co-evolution there is a strong link between virus and host genomes, and parts of the genome are probably ancient retroviral sequences \autocite{boekeRetrotransposonsEndogenousRetroviruses1997,kojimaViruslikeInsertionsSequence2021}, it has been estimated that 1\% to 8\% of the human genome is composed of these endogenous retroviral sequences \autocite{lowerVirusesAllUs1996,griffithsEndogenousRetrovirusesHuman2001}
\end{itemize}

great diversity in viruses (support and replication strategy form the basis of the Baltimore virus classification system \autocite{baltimoreExpressionAnimalVirus1971} , still used today \autocite{kooninBaltimoreClassificationViruses2021} ):
...
\item
  ...
  \item
    ...
    \begin{itemize}
    \item
      Double Stranded: like Herpesvirus \autocite{mcgeochTopicsHerpesvirusGenomics2006,boehmerHerpesVirusReplication2003}
    \item
      Single Stranded: Papillomavirus \autocite{brentjensHumanPapillomavirusReview2002}
    \item
      circular DNA: Hepatitis B \autocite{kayHepatitisVirusGenetic2007}
    \end{itemize}
  \item
    RNA: HIV/SARS-CoV (about 70\% of human pathogenic viruses are RNA viruses \autocite{domingoRNAVirusGenomes2018}
    ...
    \item
      ...
      \begin{itemize}
      \item
        positive strand: Hep C \autocite{simmondsVariabilityHepatitisVirus1995} / polio \autocite{wimmerGeneticsPoliovirus1993,racanielloOneHundredYears2006}
      \item
        negative strand: Influenza, Measles \autocite{paleseNegativestrandRNAViruses1996}
      \end{itemize}
    \item
      Double stranded: Rotavirus \autocite{parasharRotavirus1998}
    \end{itemize}
  \end{itemize}
\item
  Replication strategies \autocite{domingoVirusEvolution2014}:

  \begin{itemize}
  \item
    RNA -\textgreater{} RNA: SARS-CoV, Hep C, Dengue
  \item
    RNA -\textgreater{} DNA -\textgreater{} RNA: HIV-1
  \item
    DNA -\textgreater{} DNA: Papillomavirus, Smallpox
  \item
    DNA -\textgreater{} RNA -\textgreater{} DNA: Hep B
  \end{itemize}
\item
  ...
  \begin{itemize}
  \item
    17nm for plant satellites \autocite{pyleChapter58Biology2017} to Mimivirus a giant virus of 400nm \autocite{raoult2megabaseGenomeSequence2004}.
  \item
    860 bp for \emph{Circovirus SFBeef} to 2.5 Mbp for \emph{Pandoravirus salinus} \autocite{campillo-balderasViralGenomeSize2015}
  \end{itemize}
\item
  Shape of the virus :\autocite{cannVirusStructure2015}
...
\hypertarget{what-is-hiv}{%
\section{What is HIV ?}\label{what-is-hiv}}
...
\item
  ...
  \begin{itemize}
  \item
    Sexual activity is the largest case contributor \autocite{hladikSettingStageHost2008,shawHIVTransmission2012}:

    \begin{enumerate}
    \def\labelenumi{\arabic{enumi}.}
    \item
      Female genital tract (12.6 million)
    \item
      Male genital tract (10.2 million)
    \item
      Male/Female lower GI tract (3.9 million)
    \item
      Blood stream (2.6 million)
    \end{enumerate}
  \end{itemize}
\item
  It causes AIDS by penetrating CD4 cells of the host immune system, using them to replicate and destroying them in doing so \autocite{weissHowDoesHIV1993}
\item
  the infection typically starts out with an asymptomatic phase that can last years, followed by a decrease in CDA cells that progresses to AIDS \autocite{melhuishNaturalHistoryHIV2018}. When the CD4 cell count is low opportunistic diseases like tuberculosis, pneumonia or meningitis can more easily infect the host and lead to death.
\end{itemize}
...
\begin{itemize}
\item
  Massive global pandemic, 2010 numbers \autocite{worldhealthorganizationGlobalReportUNAIDS2010}:

  \begin{itemize}
  \item
    approx 33 million people were living with HIV, 2.6 million new infections in HIV and 1.8 millions AIDS deaths.
  \item
    This is decreasing with the 2020 numbers reporting 680,000 deaths and 1.5 million new infections \autocite{GlobalHIVAIDS}.
  \item
    70\% of the new infections are from sub-saharan africa \autocite{worldhealthorganizationGlobalReportUNAIDS2010}
  \end{itemize}
\item
  Discovered simultaneously by Françoise Barré-Sinoussi, and Luc Montagnier and Robert Gallo in 1983 \autocite{barre-sinoussiIsolationTlymphotropicRetrovirus1983,galloIsolationHumanTcell1983}
\item
  VIH-1 vs VIH-2. VIH-1 is more transmissible and is largely responsible for the global pandemic \autocite{gilbert2003}, in fact in Africa HIV-1 infections are rising where HIV-2 are declining \autocite{vanderloeffSixteenYearsHIV2006}
\item
  VIH-1 From Simian Immunodeficiency viruses most likely \autocite{sharpOriginsHIVAIDS2011}, probably from a chimpanzee \autocite{gaoOriginHIV1Chimpanzee1999,hamelTwentyYearsProspective2007}.
\item
  VIH-2 also from a SIV probably from sooty mangabey \autocite{hirschAfricanPrimateLentivirus1989,gaoHumanInfectionGenetically1992,chenGeneticCharacterizationNew1996}
\item
  There is not 1 lineage: Different groups (MNOP) correspond to distinct crossing over events (from chimp to human)
\item
  Group M is mostly responsible for the pandemic, estimated Last common ancestor of group M estimated between 1910 to 1931 \autocite{korberTimingAncestorHIV12000,worobeyDirectEvidenceExtensive2008,rambautCausesConsequencesHIV2004}, and probably originated in what is now the DRC \autocite{worobeyDirectEvidenceExtensive2008,vidalUnprecedentedDegreeHuman2000,fariaEarlySpreadEpidemic2014}.
\item
  In group M there are 9 subtypes (A to K) \autocite{mccutchanGlobalEpidemiologyHIV2006,sharpOriginsHIVAIDS2011}
\item
  There is recombination \autocite{robertsonRecombinationAIDSViruses1995} which complicates phylogenetic analyses \autocite{posadaRecombinationEvolutionaryGenomics2002}, and are a pathway to resistance and complicate vaccine development \autocite{taylorChallengeHIV1Subtype2008}.
\item
  There are 118 identified HIV-1 CRFs currently in the Los Alamos National Laboratory HIV sequence database \autocite{HIVCirculatingRecombinant} (1 for HIV-2). There are also unique recombinant forms
\item
  While subtype C represented almost half of global infections from 2004 to 2007, subtype B is the major subtype in richer countries in North America and Western Europe \autocite{hemelaarGlobalTrendsMolecular2011} where sequencing efforts are more common. This accounts for an over-representation of subtype B sequences in the LA database where 54\% of sequences are of the B subtype and only 15\% are C \autocite{DistributionAllHIV1}.
\end{itemize}

\hypertarget{life-cycle-of-hiv}{%
\subsection{Life cycle of HIV}\label{life-cycle-of-hiv}}

\begin{itemize}
\tightlist
\item
  Cycle \autocite{freedHIV1Replication2001,fergusonHIV1ReplicationCycle2002} shown in Figure \ref{fig:hivCycle}:

  \begin{enumerate}
  \def\labelenumi{\arabic{enumi}.}
  \item
    Virion attaches to CD4 host cell through membrane proteins
  \item
    Membrane fusion and entry of viral material into cell
  \item
    Reverse transcription of viral RNA into viral DNA (RT)
  \item
    integration of viral DNA into host DNA (IN)
  \item
    transcription of viral DNA into viral RNA
  \item
    translation of viral RNA into polyprotein
  \item
    cleaving of polyprotein to create mature viral proteins (PR)
  \item
    assembly of viral RNA + mature viral proteins into new virion
  \item
    Exit of new virion
  \end{enumerate}
\end{itemize}
...
\begin{itemize}
\item
  9 genes coding for 15 proteins \autocite{frankelHIV1FifteenProteins1998}, structure visible in Figure \ref{fig:hivStructure}:

  \begin{itemize}
  \item
    gag polyprotein: -\textgreater{} structural proteins
    ...
    \item
      p6: promotes budding / virion release \autocite{gottlingerEffectMutationsAffecting1991,huangP6GagRequiredParticle1995}, largely unstructured
    \end{itemize}
  \item
    pol polyprotein: -\textgreater{} essential functional proteins
    ...
    \end{itemize}
  \item
    env polyprotein: -\textgreater{} envelope

    \begin{itemize}
    \item
      SU (surface)/gp 120: responsible for binding to CD4 cells \autocite{bourHumanImmunodeficiencyVirus1995}
    \item
      TM (transmembrane)/gp 41: together with gp120 causes membrane fusion, anchors SU to the viral particle \autocite{hernandezViruscellCellcellFusion1996}
    \end{itemize}
  \item
    regulatory elements:
    ...
    \begin{itemize}
    \item
      tat (trans-activator of transcription) (!!) activates transcription -\textgreater{} essential promoter and leads to longer transcripts \autocite{jonesControlRnaInitiation1994}
    \item
      rev (Regulator of viron) (!!) helps RNA get out of nucleus for translation \autocite{hopeViralRNAExport1997}
    \item
      nef (Negative regulatory factor) (accs) prevents host cell from producing defence proteins (CD4) and boosting infectivity \autocite{mangasarianMultifacetedRoleHIV1997}
    \item
      vif (viral infectivity factor) (accs) probably boosts infection as mutants have less infectivity \autocite{cohenRoleAuxiliaryProteins1996}
    \item
      vpu (viral prot U) (accs) helps during release \autocite{lambVpuVprHuman1997,cohenRoleAuxiliaryProteins1996}
    \item
      vpr (viral prot R) (accs), prevents mitosis and helps entry in nucleus of host \autocite{emermanHIV1VprCell1996}
    \end{itemize}
  \item
    A 10th gene ?
    ...
    \begin{itemize}
    \item
      First suggested in 1988 \autocite{millerHumanImmunodeficiencyVirus1988}
    \item
      overlaps env and is read in the reverse sense than the rest of the genome
    \item
      Under selective pressure and correlated with the spread of the pandemic (specific to HIV-M) \autocite{cassanConcomitantEmergenceAntisense2016}
    \item
      under active research \autocite{savoretPilotStudyHumoral2020}
    \end{itemize}
  \end{itemize}
\end{itemize}
...
The images for HIV-1 structures used in Figure \ref{fig:hivStructure} were obtained from: \href{https://cdn.rcsb.org/pdb101/learn/resources/structural-biology-of-hiv/index.html\#}{https://cdn.rcsb.org/pdb101/learn/resources/structural-biology-of-hiv/}. They are licensed under a Creative Commons By 4.0 license which allows reuse and adaptation for non commercial use.
