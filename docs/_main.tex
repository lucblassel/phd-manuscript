...
\item
  ...
  \item
    ...
    \begin{itemize}
    \item
      Better for analyzing complex and repetitive parts of the genome, and were used for assembly \textbf{{[}CITE{]}}, structural variant detection \textbf{{[}CITE{]}}, etc\ldots{}
    \item
...
\item
  Used in many software tools for long reads \autocite{auImprovingPacBioLong2012,nurkHiCanuAccurateAssembly2020,liMinimap2PairwiseAlignment2018,ruanFastAccurateLongread2020,shafinNanoporeSequencingShasta2020,sahlinNovoClusteringLongRead2020,ekimMinimizerspaceBruijnGraphs2021} and even pyrosequencing reads \autocite{millerAggressiveAssemblyPyrosequencing2008}
\end{itemize}
...
\begin{itemize}
\item
  \textbf{Sequencing is an active field:} new methods, both in short reads (ultima) and long reads (illumina infinity), as well as improving existing technologies
\item
...
\item
  ...
  \end{itemize}
\item
  Example\ldots{} (can be at the same time as the algo presentation)
\item
...
review \autocite{chaoDevelopmentsAlgorithmsSequence2022}, we need heuristics.
...
\item
  ...
  \item
    Software: HHsearch \autocite{sodingProteinHomologyDetection2005}, HMMer \autocite{finnHMMERWebServer2011}, MCALIGN2 \autocite{wangMCALIGN2FasterAccurate2006},
  \end{itemize}
\item
  General idea:
  ...
  \item
    Many ways of finding seeds \autocite{sunChoosingBestHeuristic2006}
  \end{itemize}
\end{itemize}
...
\item
  ...
  \item
    Diamond \autocite{buchfinkFastSensitiveProtein2015}
  \item
    Specific data structures Particularly used when aligning NGS data \autocite{liSurveySequenceAlignment2010}:
    ...
    \item
      ...
      \begin{itemize}
      \item
        In some cases when using suffix trees take too much memory we can use an FM indiex which is based on the burrows wheeler transform \autocite{burrowsBlockSortingLosslessData1994}.
      \item
  ...
  \end{itemize}
\item
  You can also find seeds with Fast Fourier Transforms (FFT) as in MAFFT \autocite{katohMAFFTNovelMethod2002} by quickly computing correlations with FFT to find homologous regions.
\end{itemize}

\hypertarget{msa-short}{%
\section{MSA (short)}\label{msa-short}}
...
guide tree, clustering of sequences then refine alignment. Good heuristic but with larger datasets, becomes harder. \autocite{fengProgressiveSequenceAlignment1987}

\hypertarget{hmms-profile-alignments-pairwise}{%
\subsection{HMMs / profile alignments / pairwise}\label{hmms-profile-alignments-pairwise}}

Example of COVID where homology is high so we can get away with using HMMS / pairwise to root sequence. \emph{(point to appendix with covid align ?)}

\hypertarget{other-optimization-methods-short}{%
\subsection{Other optimization methods (short)}\label{other-optimization-methods-short}}
...
\hypertarget{the-case-of-mapping}{%
\section{The case of Mapping}\label{the-case-of-mapping}}
...
\item
  ...
  \begin{itemize}
  \item
    Repetitive regions (centromeres, telomeres) make it hard \autocite{alserTechnologyDictatesAlgorithms2021}
  \item
...
\item
  ...
  \begin{itemize}
  \item
    benchmarks/review \autocite{schbathMappingReadsGenomic2012,hatemBenchmarkingShortSequence2013}
  \item
    Algorithms in short read mapping \autocite{canzarShortReadMapping2017} (basically same thing as speeding up section)
  \item
    Hardware accelerated \autocite{olsonHardwareAccelerationShort2012}
  \end{itemize}
\item
  Long read mapping:
  ...
  \tightlist
  \item
    software: winnowmap \autocite{jainWeightedMinimizerSampling2020}, winnowmap2 \autocite{jainLongreadMappingRepetitive2022} and tandemtools \autocite{mikheenkoTandemToolsMappingLong2020} for repetitive regions.
  \end{itemize}
\item
  Mapping quality:
  ...
  \end{itemize}
\item
  Benchmarking is of interest of field \autocite{brindaRNFGeneralFramework2016}
\end{itemize}

\printbibliography[segment=\therefsegment,heading=subbibintoc,title={References for chapter \thechapter}]

\hypertarget{contribution-1-improving-local-alignment-by-exploring-a-sequence-transformation-space}{%
\chapter{Contribution 1: Improving local alignment by exploring a sequence transformation space}\label{contribution-1-improving-local-alignment-by-exploring-a-sequence-transformation-space}}
...
\hypertarget{replication-cycle-of-hiv}{%
\subsection{Replication cycle of HIV}\label{replication-cycle-of-hiv}}
...
When on ART, virus evolves under selectuve pressure and develops resitance -\textgreater{} treatment failure.

\hypertarget{how-does-art-work}{%
\subsection{How does ART work}\label{how-does-art-work}}

target the proteins, RT, PR, IN (small history of ART)

\hypertarget{different-types-of-resistance}{%
\subsection{different types of resistance}\label{different-types-of-resistance}}
...
Transmitted DRMS can be very serious , \ldots{} however fitness cost, \ldots{}
...
\tightlist
\item
  Consortiums / HIVDB, UK-CHIC, \ldots{}
\item
  stat tests
