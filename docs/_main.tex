...
\fi
\usepackage[style=authoryear,backref=true,refsegment=chapter,defernumbers=true,sorting=nyt,maxnames=3,maxbibnames=3,minnames=2,uniquelist=false]{biblatex}
...
The first true sequencing method was developed in 1977 \autocite{sangerDNASequencingChainterminating1977}. Sanger \emph{et al.} devised a simple method to read the sequence of nucleotides that make up a DNA sequence.
...
\textbf{\emph{I need to speak of performance / throughput of these methods here.}}
...
\hypertarget{how-do-we-deal-with-errors}{%
\subsection{How do we deal with errors ?}\label{how-do-we-deal-with-errors}}

\hypertarget{error-correction-methods}{%
\subsubsection{Error correction methods}\label{error-correction-methods}}
...
\subsubsection{More accurate sequencing methods}\label{more-accurate-sequencing-methods}}
...
\begin{itemize}
\item
  On the CHM13 whole human genome assembly v1.1, 29.7\% of the 3Gb are in homopolymers (i.e.~are identical to the preceding nucleotide in the sequence) Longest HP is 86.
\item
...
\item
  ...
  \emph{Short schematic example of HPC with example where it removes an error ? (don't talk about mapping yet)}
\item
...
Usually done with dynamic programming by assigning a score to mismatches, indels, \ldots{} and then choosing the alignment with minimal score.

short example of local alignment

gap open, gap extend, etc\ldots{}
...
\hypertarget{how-do-we-speed-up-local-alignment}{%
\section{How do we speed up local alignment ?}\label{how-do-we-speed-up-local-alignment}}
