...
The selection was performed on a 0.5x coverage read set, simulated from a whole human genome assembly \autocite{nurk2022}.The transformed reads were mapped to the transformed reference using \texttt{minimap2} and \texttt{paftools\ mapeval} \autocite{liMinimap2PairwiseAlignment2018} was used to compute a mapping error rate.
...
The 58 selected MSRs, as well as HPC and the identity transformation (denoted as \emph{raw}), were then evaluated on larger read-sets simulated from 3 whole genome references: a whole human genome assembly \autocite{nurk2022}, a whole \emph{Drosophila melanogaster} genome assembly \autocite{adamsGenomeSequenceDrosophila2000} and a synthetic centromeric sequence \autocite{mikheenkoTandemToolsMappingLong2020} (see \protect\hyperlink{sec:methods}{STAR Methods} for more details).
...
Recently, machine learning methods have been increasingly applied to the process of alignment. Using machine learning methods to learn an alignment method, might lead to alignments methods with less design biases.
...
It is important to note that while these transformer models are very powerful and useful in practice, their complexity and size makes them very hard to study and understand what they actually learns. There is work to peer inside this ``black box'', notably by interpreting the learned attention maps \autocite{vigBERTologyMeetsBiology2021} and decipher biologically relevant information contained within.
...
The \texttt{DEDAL} model \autocite{llinares-lopezDeepEmbeddingAlignment2022} implements similar ideas. It predicts matching, gap-open and gap-extend PSSMs form a pair of sequences, that can be used in a classical alignment method, in this case a SW algorithm. In this model, a transformer-based embedding network is used to embed each residue of both sequence. Then each possible pair of embedded residues from both sequences is used to predict specific gap-open, gap-extend and match scores used to build the PSSMs. The DEDAL model is trained on three tasks at once:
