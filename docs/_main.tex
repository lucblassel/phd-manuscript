...
There are two approaches for for pairwise alignment \autocite{sungAlgorithmsBioinformaticsPractical2011}: \emph{global alignment} where the entirety of both sequences is used when aligning them, and \emph{local alignment} where we only seek to find regions in each sequence that are most similar to each other. Global alignment is used when the two sequences are expected to be quite similar (e.g.~comparing two related proteins), whereas local alignment is mostly used when we expect the sequences to be fairly different but with highly similar regions, like genomes of two distantly related species that share a highly conserved region. Sometimes people refer to a \emph{semi-global alignment}, where two sequences are globally aligned but indels at the end and beginning of each sequence are not penalized, this can be useful to detect overlap between tow sequences or align two sequences of very different sizes.
...
One of the problems with such an approach is the size of the index, indeed storing all the \(k\)-mers of a length \(n\) sequence would require \((n-k + 1)\cdot k\) characters as the hash table keys. This space constraint is acceptable for very large scale homology search on hosted web services such as NCBI BLAST, however on a personal computer this can easily exceed memory capacity. Storing the hash table on disk has drastic consequences on query times, therefore methods to reduce the storage needs of these data structures were developed.
...
One common other method for creating multiple alignment, wither through profile-profile alignment or sequence-profile alignments are HMMs. Several tools HMMs to generate an alignment such as HHMer \autocite{finnHMMERWebServer2011}, MSAProbs \autocite{liuMSAProbsMultipleSequence2010} or COVID-align \autocite{lemoineCOVIDAlignAccurateOnline2020}. In some cases, the HMM based approach has similar performance to clustalW \autocite{eddyMultipleAlignmentUsing}.
...
Here we have extra knowledge -\textgreater{} labels and we can use his information to train the model.
...
\begin{itemize}
\item
  predict a quantified resistance level of a virus to a drug (fold change) \autocite{steinerDrugResistancePrediction2020a}
\item
  predict drug response in cancer (i.e.~IC\textsubscript{50}) \autocite{ammad-ud-dinSystematicIdentificationFeature2017}, they use genome-wide features pre-extracted from sequences and not sequence directly.
\item
  predict angles or contact maps in protein structure -\textgreater{} very active field \autocite{noeMachineLearningProtein2020,pearceSolutionProteinStructure2021,tunyasuvunakoolHighlyAccurateProtein2021,chengMachineLearningMethods2008,alquraishiMachineLearningProtein2021} either directly from MSAs \autocite{jumperHighlyAccurateProtein2021} or from derived features like contact maps \autocite{wangAccurateNovoPrediction2017}
\item
  Score MSAs \autocite{ortunoComparingDifferentMachine2015}
\item
  predicting protein fitness for \emph{in silico} protein engineering \autocite{wittmannAdvancesMachineLearning2021,yangMachinelearningguidedDirectedEvolution2019,liCanMachineLearning2019}
\item
  predicting gene expression levels from SNPs (meaning we need alignments\ldots{} to a reference) \autocite{xieDeepAutoencoderModel2017}
\end{itemize}
...
\begin{itemize}
\item
  resistant virus or not \autocite{hagaMachineLearningbasedTreatment2020,zazziPredictingResponseAntiretroviral2012} (also Chapter \ref{HIV-paper}) also many studies on antimicrobial resistance \autocite{renPredictionAntimicrobialResistance2022,kimMachineLearningAntimicrobial2022}
\item
  Cellular localization of protein \autocite{weiPredictionHumanProtein2018}
\item
  Secondary structure of amino acid in protein sequence \autocite{jonesProteinSecondaryStructure1999} (\(\alpha\), \(\beta\) or other)
\item
  localized gene expression in cells \autocite{kelleyBassetLearningRegulatory2016}
\item
  splice site detection \autocite{ratschLearningInterpretableSVMs2006}
\item
  Methylation site prediction \autocite{wangPredictingDNAMethylation2016}
\item
  Protein function prediction \autocite{wangProteinSequenceProtein2017}
\end{itemize}
...
\begin{itemize}
\item
  clustering, align all pairs of sequences for clustering \autocite{zoritaStarcodeSequenceClustering2015}, phylogenetics could be considered as a specific type of clustering \autocite{balabanTreeClusterClusteringBiological2019}.
\item
  predict mutational effects \autocite{hopfMutationEffectsPredicted2017}
\item
  Predict recombination hotspots in humans with a MaxLik approach \autocite{castroModelSelectionApproach2018}
\item
  Approximating the edit distance and bypassing the need for alignment \autocite{corsoNeuralDistanceEmbeddings2021}
\item
  dimensionality reduction:

  \begin{itemize}
  \item
    traditionally apply PCA to some type of matrix, like distance matrix obtained from aln \autocite{haschkaMNHNTreeToolsToolboxTree2021} or other matrices like gene expression
  \item
    Also can be used for clustering \autocite{ben-hurDetectingStableClusters2003,KmeansClusteringPrincipal,casariSequencespaceToolFamily1995,clampJalviewJavaAlignment2004}
  \item
    Work done to compute PCA directly on alignment \autocite{konishiPrincipalComponentAnalysis2019}
  \end{itemize}
\end{itemize}
...
\item
  ...
  \begin{itemize}
  \item
    Predict P/P interactions \autocite{townshendEndtoEndLearning3D2019}
  \item
    microRNA target prediction \autocite{leeDeepTargetEndtoendLearning2016}
  \end{itemize}
\item
  ...
  \begin{itemize}
  \item
    Used to find useful info in disordered protein regions \autocite{luDiscoveringMolecularFeatures2022}
  \item
    Protein language models: \autocite{elnaggarProtTransCrackingLanguage2021}
  \end{itemize}
\item
  ...
  \begin{itemize}
  \item
    used to predict drug-protein interactions \autocite{xiaSemisupervisedDrugproteinInteraction2010}
  \item
    and transmembrane protein topology \autocite{tamposisSemisupervisedLearningHidden2019}
  \end{itemize}
\end{itemize}
...
\hypertarget{the-general-supervised-learning-paradigm}{%
\subsection{The general supervised learning paradigm}\label{the-general-supervised-learning-paradigm}}
...
\item
  ...
  \item
    Important that each of these datasets are completely separate -\textgreater{} avoid data leaks \autocite{kaufmanLeakageDataMining2011}.
  \end{itemize}
\item
  Learning -\textgreater{} optimization process:
  ...
  \end{itemize}
\item
  Some semantics (however in a lot of literature cost = loss, e.g. \autocite{Goodfellow-et-al-2016}).
  ...
  \end{itemize}
\item
  Example of a classical cost function, many loss functions in ML \autocite{wangComprehensiveSurveyLoss2022}:

  \begin{itemize}
  \item
    \(y_i\) = i\textsuperscript{th}real value of the target, \(\hat{y}_i\) = i\textsuperscript{th}predicted value, for binary classification, \(p_i\) = probability that i\textsuperscript{th} observation is in positive class.
  \item
  ...
  \end{itemize}
\item
  Some performance measures many performance measures (especially in classification \autocite{jiaoPerformanceMeasuresEvaluating2016}) :
...
\begin{itemize}
\item
  Overfitting \autocite{hastieElementsStatisticalLearning2009} :

  \begin{itemize}
  \item
    What is it ?
  \item
    How to detect it -\textgreater{} schematic here of training and testing loss ref to Figure \ref{fig:overfitting}
  \end{itemize}
\item
...
\item
  ...
  \end{itemize}
\item
  More sophisticated is linear and logistic regression, for regression and classification respectively. \autocite{hastieElementsStatisticalLearning2009}
  ...
  \item
    To avoid overfitting- \textgreater{} regularization where the either ridge \autocite{hoerlRidgeRegressionBiased1970} or lasso \autocite{tibshiraniRegressionShrinkageSelection1996} .
  \end{itemize}
\item
  Naive Bayes classifier \autocite{hastieElementsStatisticalLearning2009}
  ...
  \item
    Retains pretty good predictive power \autocite{zhangOptimalityNaiveBayes,rishEmpiricalStudyNaive} despite the assumptions being violated
  \end{itemize}
\item
  Other simple methods like KNN are also often used.
\end{itemize}

\hypertarget{more-complex-methods-machine-learning}{%
\subsection{More complex methods, machine learning}\label{more-complex-methods-machine-learning}}
...
\item
  ...
  \begin{itemize}
  \item
    Optimal separation hyperplane -\textgreater{} linear separation \autocite{vapnikEstimationDependencesBased1982}
  \item
    Can perform non-linear classification with the kernel trick in the early 90's \autocite{boserTrainingAlgorithmOptimal1992,cortesSupportvectorNetworks1995}
  \item
    adapted to regression \autocite{druckerSupportVectorRegression1996}
  \end{itemize}
\item
  ...
  \begin{itemize}
  \item
    Introduced by Leo Breiman \autocite{breimanRandomForests2001}
  \item
    Based on the CART decision tree method \autocite{breimanClassificationRegressionTrees1983}
  \item
  ...
  \item
    Good performance compared to other methods presented before \autocite{caruanaEmpiricalComparisonSupervised2006a}
  \end{itemize}
\item
...
\hypertarget{general-purpose-embeddings}{%
\subsection{General purpose embeddings}\label{general-purpose-embeddings}}

review \autocite{potdarComparativeStudyCategorical2017}
...
\item
  ...
  \tightlist
  \item
    pb -\textgreater{} assigning artificial order to a purely categorical variable, used in \autocite{steinerDrugResistancePrediction2020a}
  \end{itemize}
\item
  BItwise labeling -\textgreater{} encode each character with \(n\) bits, for nucleotides 2bits if no gaps (A=00, C=01, G=10, T=11) \autocite{dufresneKmerFileFormat2022,wrightUsingDECIPHERV22016} for AAs 5 bits necessary explored in \autocite{zamaniAminoAcidEncoding2011}

  \begin{itemize}
  \item
    still a problem of order, for aas used in hashing and bloom filters \ldots{}
  \item
  ...
  \end{itemize}
\item
  One-Hot encoding, widely used: a variable with \(d\) levels (for DNA \(d=4\)) is transformed to a length \(d\) sparse binary vector, when the variable is equal to the \(i^{th}\) level then the resulting vector has a 1 set in the \(i^{th}\) position. (also called orthonormal \autocite{singhEvolutionaryBasedOptimal2018})

  \begin{itemize}
  \item
    Used in Chapter \ref{HIV-paper} and other papers \autocite{budachPyssterClassificationBiological2018},
  \item
  ...
  \item
    Performance between OH and Ordinal can be quite similar \autocite{choongEvaluationConvolutionaryNeural2017} but easily interpretable (important in Bio)
  \item
    Has been used for encoding protein sequences as early as 1988 \autocite{qianPredictingSecondaryStructure1988}
  \end{itemize}
\end{itemize}
...
\tightlist
\item
  Many general purpose categorical encodings, library in Python \autocite{mcginnisScikitLearnContribCategoricalEncodingRelease2018}
\end{itemize}

\hypertarget{biological-sequence-specific-embeddings}{%
\subsection{Biological sequence-specific embeddings}\label{biological-sequence-specific-embeddings}}

These embeddings were inspired by physico-chemical properties, many encodings in \autocite{zamaniAminoAcidEncoding2011}:

\begin{itemize}
\item
  AAIndex embedding, based on the the AAIndex database \autocite{kawashima2008},

  \begin{itemize}
  \item
    each amino acid has a number real-valued physico-chemical properties so you can select a subset of them.
  \item
    A subset of properties identified as informative \autocite{liPredictionProteinStructural2008}
  \item
    Available in the iFeature python package \autocite{chenIFeaturePythonPackage2018}
  \item
    Also, combine features from AAIndex with PCA to get ensemble features and use top components \autocite{nanniNewEncodingTechnique2011}
  \end{itemize}
\item
  Groups:
  ...
  \begin{itemize}
  \item
    Based on the Venn diagram defined by Taylor \autocite{taylorClassificationAminoAcid1986}, grouping AAs by 9 properties (hydrophobic, polar, small, \ldots)
  \item
    We can get a 9-D vector for each residue
  \item
    An encoding based on that was used as early as 1987 to predict protein secondary structure \autocite{zvelebilPredictionProteinSecondary1987}
  \item
    Later 5 more groups defined \autocite{kremerMethodSystemComputer2009}
  \end{itemize}
\item
  BLOMAP \autocite{maetschkeBlomapEncodingAmino2005}:

  \begin{itemize}
  \item
    Non-linear projection of the BLOSUM62 substitution matrix to encode an amino acid in 5 dimensions
  \item
    Used to predict cleavage sites of HIV-1 protease \autocite{singhEvolutionaryBasedOptimal2018}
  \end{itemize}
\item
  Others:
  ...
  \begin{itemize}
  \item
    frequencies of amino acids -\textgreater{} very coarse (but low dimensional)
  \item
    \(k\)-mer frequencies, often reffered to as \(n\)-gram encoding \autocite{sahaNovelApproachFind2019}, often \(n=2\) since as \(k\)/\(n\) grow the dimensions grow exponentially -\textgreater{} with 20 AAs and \(20^n\) dimensions per sequence.
  \item
    OETMAP, extension of BLOMAP \autocite{gokNewFeatureEncoding2013}
  \item
    Codon graph encoding proposed in \autocite{zamaniAminoAcidEncoding2011}, where an AA is represented by a directed graph of all the codons that make up the AA. The 4x4 adjacency matrix is converted into a length 16 vector representing a single AA.
  \end{itemize}
\end{itemize}
...
\begin{itemize}
\item
  Applications in BIoinfo \autocite{lochelChaosGameRepresentation2021}
\item
  Applied for phylogenetics:

  \begin{itemize}
  \item
    classify SARS-CoV 2 sequences \autocite{cartesAccurateFastClade2022} w/ DL
  \item
    \autocite{niApplyingFrequencyChaos2021}
  \end{itemize}
\item
  Predict AMR \autocite{renPredictionAntimicrobialResistance2022}
\end{itemize}
...
\item
  Haven't gone over DL -\textgreater{} chapter 7
\end{itemize}
...
\item
  ...
  \item
    ...
    \item
      ...
      \begin{itemize}
      \item
        Intuitively learn structure of proteins \autocite{raoTransformerProteinLanguage2020,rivesBiologicalStructureFunction2019}
      \item
        Learn mutational effects \autocite{meierLanguageModelsEnable2021}
      \item
        Evolutionary characteristics \autocite{hieEvolutionaryVelocityProtein2022}
      \end{itemize}
    \item
      To counter the space limitations (i.e.~sequence length limitations) induced by attention, other types of transformers used, with linear scale attention maps not quadratic \autocite{choromanskiMaskedLanguageModeling2020}
    \end{itemize}
  \item
    Include information from MSA directly in embedding\autocite{caiGenomewidePredictionSmall2020}: transform aligned sequence in to tokens -\textgreater{} use ALBERT to embed tokens
  \item
    MSA Transformer \autocite{raoMSATransformer2021} that extends attention to include aligned residues from an input MSA as well.

    \begin{itemize}
    \item
      Similarly: learn on profiles derived from MSAs \autocite{sturmfelsProfilePredictionAlignmentBased2020} as a pre-training task for protein language models
    \item
      Learn a protein structure model (potts model) directly on the MSA with a mechanism similar to attention \autocite{sercuNeuralPottsModel2021}
    \end{itemize}
  \end{itemize}
\item
  Powerful but hard to interpret what the model actually learns. i.e.~``black box'' but some work is being done to interpret attention maps \autocite{vigBERTologyMeetsBiology2021}
\end{itemize}
