...
[@zhangComparisonPrecisionSensitivity2005]. Many methods have been used
to predict a resistance level: Support Vector Machines (SVMs)
[@beerenwinkelGeno2phenoEstimatingPhenotypic2003], k-Nearest Neighbors
(KNN) and Random Forests (RFs) [@shenAutomatedPredictionHIV2016], and
more recently Artificial Neural Networks (ANNs)
[@yuPredictionHIVDrug2014; @sheikamamuddyImprovingFoldResistance2017].
...
[@riemenschneiderExploitingHIV1Protease2016; @heiderMultilabelClassificationExploiting2013]
and also ANNs [@draghiciPredictingHIVDrug2003]. Most recently Steiner
_et al._ [@steinerDrugResistancePrediction2020] have used Deep Learning
...
[@steinerDrugResistancePrediction2020] have checked that known DRM
...
```{r}
figClassificationPerfCaption <- "**Classifier Performance on UK and African datasets.**  
...
```{r}
tabMutationsCaption <- "**Analysis of new potential RAMs.**  
...
dt <- read_csv("tables/HIV-DRMs/Table3.short.csv")

labels <- data.frame(
  col_keys = c("mutation","min","avg","B62","Ucount","Unt","Unr", "p‑value"),
  what = c("mutation","codon distance","codon distance","B62","UK","UK","UK","p‑value"),
  measure = c("mutation","min","avg","B62", "count","RR(new,treatment)","RR(new,any RAM)","p‑value"),
...
flextable(dt) %>%
  ...
  set_caption(caption=tabMutationsCaption, style="caption")
```
...
```{r}
figStructureCaption <- "**Structure of HIV-1 RT with highlighted important sites.**  
