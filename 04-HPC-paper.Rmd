...
**1** Sequence Bioinformatics, Department of Computational Biology, Institut Pasteur, Paris,
France\
...
4) [@dohmBenchmarkingLongreadCorrection2020]. Many algorithmic problems,
such as alignment, become trivial if not for sequencing
errors [@gusfieldbook]. Error correction can often decrease the error
...
*homopolymer compression* (abbreviated HPC). HPC simply transforms runs
...
data [@sahlinErrorCorrectionEnables2021]. HPC is used in alignment, e.g.
by the widely used minimap2 aligner [@liMinimap2PairwiseAlignment2018].
HPC is also used in long-read assembly, e.g.
HiCanu [@nurkHiCanuAccurateAssembly2020],
SMARTdenovo [@liuSMARTdenovoNovoAssembler2021], or
mdBG [@ekimMinimizerspaceBruijnGraphs2021]. HPC is also used for
clustering transcriptome reads according to gene family of
origin [@sahlinNovoClusteringLongRead2020]. Overall, HPC has been widely
...
define a more general notion which we call *mapping-friendly sequence
...
HPC. We evaluate them using two mappers (`minimap2` and `winnowmap2`) on
three simulated datasets (whole human genome, human centromere, and
whole *Drosophila* genome). We show that some of these functions provide
...
one function increased the mapping accuracy of `minimap2` by an order of
...
### Mapping-friendly sequence reductions {#sec:msr-def}
...
into Equation \@ref(eq:MSR) increasing the context that $g$ uses (i.e. setting $\ell>2$).
Formally, for a given alphabet $\Sigma$ and a context size $\ell$, a
function $T$ mapping strings to strings is said to be an *order-$\ell$
mapping-friendly sequence reduction* (abbreviated *MSR*) if there exists
...
countingMSRsCaption <- "**Representing and counting mapping-friendly sequence reductions.**\
...
\begin{figure}[t]
    ...
    \extcaption{Representing and counting mapping-friendly sequence reductions.}{
        \textbf{A}: General representation of an order-2 mapping-friendly sequence reduction as a mapping of 16 input dinucleotides, to the 4 nucleotide outputs and the empty character $\varepsilon$.
        \textbf{B}: Homopolymer compression is an order-2 MSR. All dinucleotides except those that contain the same nucleotide twice map to the second nucleotide of the pair. The 4 dinucleotides that are the two same nucleotides map to the empty character $\varepsilon$.
...
Since we aim to use MSRs in the context of sequencing data, we need to
place additional restrictions on how they handle reverse complements.
For example, given two strings $x$ (e.g. a read) and $y$ (e.g. a
substring of the reference), a mapper might check if $x = RC(y)$. When
strings are pre-processed using an MSR $f$, it will end up checking if
$f(x) = RC(f(y))$. However, $x = RC(y)$ only implies that
$f(x) = f(RC(y))$. In order to have it also imply that
$f(x) = RC(f(y))$, we need $f$ to be commutative with RC, i.e. applying
MSR then RC needs to be equivalent to applying RC then MSR. We say that
$f$ is *RC-insensitive* if for all $x$, $f(RC(x))= RC(f(x))$. Observe
that HPC is RC-insensitive.

### Restricting the space of mapping-friendly sequence reductions {#sec:enum}
...
#### Reverse complement-core-insensitive mapping-friendly sequence reductions {#sec:rc-insensitive}

Consider an MSR defined by a function $g$, as in Equation \@ref(eq:MSR).
...
*RC-core-insensitive* if the function $g$ that defines it has the
...
#### Equivalence classes of MSRs {#sec:equiv}
...
Equation \@ref(eq:MSR). An $\ell$-mer is canonical if it is the not lexicographically larger than
...
*dimension* $k$ is the number of distinct nucleotides that can be output
...
can therefore view it as a partition of $I$ into $k+1$ sets $S_0$, ...,
...
-   there exists a permutation $\pi$ of $\{1,\ldots, k\}$ such that for
    all $1 \leq i \leq k$, we have $S_i = S'_{\pi(i)}$,

-   for all $1 \leq i < j \leq k$, we have
    $\textsc{IsComp}(t(i), t(j)) = \textsc{IsComp}(t'(\pi(i)), t'(\pi(j)))$.

One can verify that this definition is indeed an equivalence relation,
i.e. it is reflexive, symmetric, and transitive. Therefore, we can
partition the set of all MSRs into equivalence classes based on this
equivalence relation. One caveat is that a single MSR defined by a
function $g$ may correspond to multiple MSRs of the form
$S_0,\ldots,S_k,t$. However, these multiple MSRs are equivalent, hence
the resulting equivalence classes are not affected. Furthermore, we can
assume that there is some rule to pick one representative MSR for its
equivalence class; the rule itself does not matter in our case.

Figure \@ref(fig:countingMSRs) shows the equivalence classes for $\ell=2$,
for a fixed partition. An equivalence class can be defined by which pair
of classes $S_i$ and $S_j$ have complementary outputs under $t$ and
$t'$. Let us define $o(k)$ as the number of equivalence classes for a
given partition and a given $k$. Then
Figure \@ref(fig:countingMSRs) shows that $o(1)=1$, $o(2)=2$ and
$o(3) = o(4) = 3$. There are thus only 9 equivalence classes for a given
partition.


```{r}
figMSR_configs_caption <- "**MSR equivalence classes for a fixed partition of the inputs.**\
...
\begin{figure}
    ...
    \extcaption{MSR equivalence classes for a fixed partition of the inputs.}{
    $S_0$ is always assigned $\varepsilon$, so it is represented by a gray node. 
    A blue link between $S_i$ and an $S_j$ denotes that $\iscomp(t(i), t(j))=\text{true}$.
...
#### Counting the number of restricted MSRs
...
1.  **Whole human genome:** This reference sequence is a whole genome
    assembly of the CHM13hTERT human cell line by the
    Telomere-to-Telomere consortium [@nurkCompleteSequenceHuman2021]. We
    used the 1.1 assembly release (Genbank Assembly ID
    [GCA_009914755.3](https://www.ncbi.nlm.nih.gov/assembly/GCA_009914755.3/)).

2.  **Whole *Drosophila* genome:** This reference sequence is a whole
    genome assembly of a *Drosophila melanogaster*, release 6.35
    (Genbank Assembly ID
    [GCA_000001215.4](https://www.ncbi.nlm.nih.gov/assembly/GCF_000001215.4/))
    [@adamsGenomeSequenceDrosophila2000].

3.  **Synthetic centromeric sequence:** This sequence was obtained from
    the `TandemTools` mapper test data
    [@mikheenkoTandemToolsMappingLong2020]. It is a simulated
...
`nanosim` [@yangNanoSimNanoporeSequence2017] with the
...
`minimap2`[@liMinimap2PairwiseAlignment2018] with the `map-ont` preset
and the `-c` flag to generate precise alignments. Although HPC is an
option in `minimap2` we do not use it, we evaluate HPC as any of the
...
`paftools mapeval` command [@liMinimap2PairwiseAlignment2018]. This tool
...
-   **MSR$_{\text{F}}$:** The MSR with the highest fraction of mapped
    reads at a mapq threshold of 0.

-   **MSR$_{\text{E}}$:** The MSR with the lowest error rate at the
    first mapq threshold for which it is "better" than HPC at mapq 60.

-   **MSR$_{\text{P}}$:** The MSR with the highest percentage of mapq
    thresholds for which it is "better" than HPC at mapq 60.

Figure \@ref(fig:topMSRs) shows the actual functions MSR$_{\text{F}}$,
MSR$_{\text{E}}$, MSR$_{\text{P}}$. An intriguing property is that they
output predominantly As and Ts, with MSR$_{\text{P}}$ assigning 2 input
pairs to the G/C output whereas MSR$_{\text{E}}$ and MSR$_{\text{F}}$
assign only one. Additionally, MSR$_{\text{E}}$ and MSR$_{\text{P}}$
both assign the {CC,GG} input pair to the deletion output $\varepsilon$
removing any information corresponding to repetitions of either G or C
from the reduced sequence. Overall this means the reduced sequences are
much more AT-rich than their raw counterparts, but somehow information
pertinent to mapping is retained.
...
\begin{figure}[h!]
    ...
    \extcaption{Graph representations of our highlighted MSRs: \msr{E},  \msr{F},  and \msr{P}.}{
        \msr{E} has the lowest error rate of among MSRs at the highest mapq threshold for which it performs better than HPC at mapq 60, \msr{F} has the highest fraction of reads mapped at mapq 60 and \msr{P} has the highest percentage of mapq thresholds for which it outperforms HPC at mapq 60.
        The grayed out nodes represent the reverse complement of input dinucleotides and outputs, as in \autoref{fig:countingMSRs}C. For example for \msr{E}, AA is mapped to T, so TT is mapped to A.}
    \label{fig:topMSRs}
\end{figure}

### Mapping-friendly sequence reductions lead to lower mapping errors on whole genomes

Across the entire human genome, at high mapping quality thresholds
(above 50), our selected 58 MSRs generally have lower mapping error rate
than HPC and raw Figure \@ref(fig:mapeval)A and
Table \@ref(tab:table-subset)). This is expected as we selected those
MSRs partly on the criteria of overperforming HPC at mapq 60.
...
Given that mapping quality is only an indication from the aligner to
estimate whether a read mapping is correct, and that we now know that
the true mapping accuracy of MSRs is high even for reported mapping
qualities that are low, we choose to compare MSR-mapped reads with lower
mapping qualities against raw or HPC-mapped reads with the highest (60)
mapping quality. By setting a different mapping quality threshold for
our MSRs ($\geq 50$) than for the HPC/raw mappings (60),
the three selected MSRs outperform both HPC and raw in terms of mapping
error rate, with similar fractions of mapped reads overall. For example
on the human genome, at mapq $\geq 50$, MSR$_{\text{F}}$,
MSR$_{\text{P}}$ and MSR$_{\text{E}}$ all map more reads than either HPC
or raw at mapq=60, and MSR$_{\text{P}}$ and MSR$_{\text{E}}$ also have
error rates an order of magnitude lower than either HPC or raw
(Table \@ref(tab:table-subset) and Figure \@ref(fig:mapeval)A).
...
-   Using the *Drosophila melanogaster* whole genome assembly as
    reference and mapping with `minimap2`

-   Using the whole human genome assembly as reference but mapping with
    `winnowmap2`(version 2.02) [@jainWeightedMinimizerSampling2020]. The
    same options as `minimap2` were used, and k-mers were counted using
    `meryl` [@rhieMerquryReferencefreeQuality2020], considering the top
    $0.02\%$ as repetitive (as suggested by the `winnowmap2` usage
    guide).
...
`mapeval` error rates (Table \@ref(tab:table-subset)). A noticeable exception is the
`winnowmap2` experiment, where a larger fraction of raw reads are mapped
...
```
```{r, tabtableSubset, label="table-subset", eval=knitr::is_html_output(), tab.topcaption=FALSE, eval=knitr::is_html_output()}
...
ft <- set_caption(ft, caption=tabtableSubsetCaption, style="caption")
...
\multicolumn{2}{l}{}     & \multicolumn{4}{c}{\minimap}                                                            & \multicolumn{4}{c}{\winnowmap}                                                          & \multicolumn{4}{c}{\minimap}                                                         \\
...
\msr{F}     & 50         & \textbf{0.938} & \textbf{\:\small$+$0\%} & 1.29e-03          & {\:\footnotesize$-$30\%} & 0.886          & {\:\footnotesize$-$1\%} & 3.82e-04          & {\:\footnotesize$-$73\%} & \textbf{0.960} & \textbf{\:\small$+$0\%} & 1.37e-03      & {\:\footnotesize$-$ 39\%} \\
\msr{E}     & 50         & 0.936          & {\:\footnotesize$+$0\%} & \textbf{1.17e-04} & \textbf{\:\small$-$94\%} & 0.820          & {\:\footnotesize$-$8\%} & \textbf{8.93e-05} & \textbf{\:\small$-$94\%} & 0.954          & {\:\footnotesize$-$0\%} & \textbf{0.00} & \textbf{\:\small$-$100\%} \\ 
\msr{P}     & 50         & \textbf{0.938} & \textbf{\:\small$+$0\%} & 4.15e-04          & {\:\footnotesize$-$78\%} & 0.845          & {\:\footnotesize$-$6\%} & 1.14e-04          & {\:\footnotesize$-$92\%} & 0.957          & {\:\footnotesize$+$0\%} & 8.11e-04      & {\:\footnotesize$-$ 64\%} \\ \bottomrule
\end{tabular}%
}
...
alignments that `paftools mapeval` reported as incorrectly mapped. For
...
`minimap2` on simulated Oxford Nanopore long reads.

We focused on reads with high mapping quality (50-60), as it is a common
practice to disregard reads with low mapping
quality [@prodanovSensitiveAlignmentUsing2020; @liNewStrategiesImprove2021; @liSyntheticdiploidBenchmarkAccurate2018].
...
recommendation made in `minimap2` to not apply HPC to ONT data. Despite
this, we newly show the benefit of using HPC (and our MSRs) with
`minimap2` on ONT data when focusing on high mapping quality reads.
...
of reads mapped and error rate, on whole human and *Drosophila
