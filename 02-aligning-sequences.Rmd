...
1.  The optimal alignment score of $S_1[1,l_1-1]$ [^aligning-sequences-1] and $S_2[1,l_2-1]$ plus the cost of a match or mismatch between the $l_1^{th}$ character of $S_1$ and the $l_2^{th}$ character of $S_2$

2.  The optimal alignment score of $S_1$ and $S_2[1,l_2-1]$ plus the cost of an indel

3.  The optimal alignment score of $S_1[1,l_1-1]$ and $S_2$ plus the cost of an indel

[^aligning-sequences-1]: Here I am using an index starting at 1 and inclusive, so $S_1[1,l_1-1]$ represents the first $l_1-1$ characters. If $S_1 = ABCD$ then $S_1[1;3]=ABC$
...
If $S_1$ represents the rows of the matrix, and $S_2$ the columns, a cell $(i,j)$ of this matrix represents the optimal alignment score between $S_1[1,i]$ and $S_2[1,j]$[^aligning-sequences-2]. In the recurrence relation described above the alignment score as dependant on the optimal alignment scores of subsequences, when filling out the dynamic programming matrix we proceed in the inverse fashion by using the scores of short subsequences to build up the scores of progressively longer sequences.

[^aligning-sequences-2]: In practice we add a empty character at the beginning of $S_1$ and $S_2$ before filling out the matrix so a cell $(i+1,j+1)$ represents the optimal alignment score between $S_1[1,i]$ and $S_2[1,j]$

We will go here through a short example showing how the NW algorithm is used to align two short sequences: `ACCTGA` and `ACGGA`. The first step is to represent the dynamic programming matrix, prefix each sequence with an empty character and label the rows of the matrix with one of the sequences and the columns with the other.
