...
The selection was performed on a 0.5x coverage read set, simulated from a whole human genome assembly [@nurkCompleteSequenceHuman2022].The transformed reads were mapped to the transformed reference using `minimap2` and `paftools mapeval` [@liMinimap2PairwiseAlignment2018] was used to compute a mapping error rate.
...
The 58 selected MSRs, as well as HPC and the identity transformation (denoted as *raw*), were then evaluated on larger read-sets simulated from 3 whole genome references: a whole human genome assembly [@nurkCompleteSequenceHuman2022], a whole *Drosophila melanogaster* genome assembly [@adamsGenomeSequenceDrosophila2000] and a synthetic centromeric sequence [@mikheenkoTandemToolsMappingLong2020] (see [STAR Methods](#sec:methods) for more details).
...
1.  **Whole human genome:** This reference sequence is a whole genome assembly of the CHM13hTERT human cell line by the Telomere-to-Telomere consortium [@nurkCompleteSequenceHuman2022]. We used the 1.1 assembly release (Genbank Assembly ID [GCA_009914755.3](https://www.ncbi.nlm.nih.gov/assembly/GCA_009914755.3/)).
